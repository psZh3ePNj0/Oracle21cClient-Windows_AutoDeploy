########################################################################################################
#	
#	FileName:		UnInstall_21c32.ps1
#	Author:			Christophe Cartwright, Oracle DBA
#	Date:			
#	Version:		1.0
#	Description:	Shell Script Vets for Oracle_Homes (from inventory.xmls)
# 					Depending on checks, Oracle 21c [32] will be Dynamically uninstalled  in Silent Mode
#
...
try{
    ...
    Start-transcript -Path "C:\Oracle21C32_UnInstall.txt"
			...
			foreach ($location in $HomeLocations)
			{
				...
				write-host 
					...
					if ($Hname -like "OraClient21Home*")	
					{
						write-host ====================================================================================================
						write-host
						write-host "Oracle21c 32bit Client installed. Proceeding to uninstall Oracle21c 32bit Client. "
						write-host
						...
						else
						{
							write-host ====================================================================================================
							write-host
							write-host "No Oracle 21c 32Bit Client Response File Generated. An issue occurred. Please investigate. Exiting."
							write-host
							write-host ====================================================================================================
							write-host
                            ...
                            if($CentralResultsDir_HostName){
							    ...
							    C:
							    Copy-Item -Path "C:\Oracle21C32_UnInstall.txt" "$CentralResultsDir\$Host_Name"
							    Remove-Item -Path "C:\Oracle21C32_UnInstall.txt" -Recurse -Force -Confirm:$false
							    exit 1618
                            }
					...
					elseif ( ($Hname -notlike "OraClient21Home*") -and ($HomeLocationsCurrent -ne $HomeLocationsCount))
					{
						...
						write-host "Continuing search for Oracle21c 32bit Client."
						write-host
						write-host ====================================================================================================
						write-host
					...
					elseif ( ($Hname -notlike "OraClient21Home*") -and ($HomeLocationsCurrent -eq $HomeLocationsCount))
					{
						...
						write-host ====================================================================================================
						write-host
						write-host "No Oracle Client 32Bit Installed. Gracefully exiting."
						write-host
						write-host ====================================================================================================
						write-host
    ...
    # for unknown errors. exit with Retry code 1618.
		...
		write-host ====================================================================================================
		write-host
		Write-Host "Error with Oracle 21c 32Bit UnInstallation."
        write-host
		write-host ====================================================================================================
		write-host
